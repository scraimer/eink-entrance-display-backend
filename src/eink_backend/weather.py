...
def _collect_data_impl(now: datetime) -> WeatherForecast:
    ...
    # TODO: Switch to unix timestamp and adjust all the datetimes locally
    ...
    time_len = len(daily_data["time"])
    for key in daily_data:
        assert (
            len(daily_data[key]) == time_len
        ), f"Error: key {key} has a different length."
    dailies: List[WeatherDaily] = []
    for i in range(time_len):
        daily = WeatherDaily(
            timestamp=datetime.strptime(daily_data["time"][i], DATE_FORMAT),
    ...
    return WeatherForecast(current=current, hourlies=hourlies, dailies=dailies)
...
def weather_report(weather_forecast: WeatherForecast, color: str):
    ...
    daily_template = Template(
        ...
        <li>
        <ul>
            <li class="black hour">$day_of_week</li>
            <li class="black temp">$apparent_temperature_min - $apparent_temperature_max &deg;C</li>
            <li class="$color icon"><img src="$icon_url_modified"/></li>
        </ul>
        </li>"""
    )
    ...
    daily = weather_forecast.dailies[0]
    hours_str += daily_template.substitute(
        **daily.__dict__,
        day_of_week=daily.timestamp.strftime("%A"),
        icon_url_modified=render.image_extract_color_channel(
            img_url=WMO_CODES[str(daily.weather_code)][
                "night" if daily.timestamp.hour > 17 else "day"
            ]["image"],
            color=color,
        ),
    )
